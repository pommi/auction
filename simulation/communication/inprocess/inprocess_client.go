...
func (client *InprocessClient) RebidThenTentativelyReserve(repAddresses []auctiontypes.RepAddress, startAuctionInfo auctiontypes.StartAuctionInfo) auctiontypes.StartAuctionBids {
	c := make(chan auctiontypes.StartAuctionBid)
	for _, repAddress := range repAddresses {
		go client.reserveAndRecastScore(repAddress.RepGuid, startAuctionInfo, c)
	}
...
func (client *InprocessClient) ReleaseReservation(repAddresses []auctiontypes.RepAddress, startAuctionInfo auctiontypes.StartAuctionInfo) {
	c := make(chan bool)
	for _, repAddress := range repAddresses {
