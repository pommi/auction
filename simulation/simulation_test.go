...
var _ = Describe("Auction", func() {
	...
	newLRP := func(processGuid string, index int, memoryMB int) rep.LRP {
		return rep.NewLRP(processGuid, index, rep.NewResource(int32(memoryMB), 1, linuxRootFSURL))
	}
	...
	newLRPStartAuction := func(processGuid string, index int, memoryMB int32) auctioneer.LRPStartRequest {
		return auctioneer.NewLRPStartRequest(processGuid, []int{index}, rep.NewResource(memoryMB, 1, linuxRootFSURL))
	}
