...
var _ = Describe("Cell", func() {
	...
	BeforeEach(func() {
		...
		state := BuildCellState("the-zone", 100, 200, 50, false, linuxOnlyRootFSProviders, []rep.LRP{
			*BuildLRP("pg-1", 0, linuxRootFSURL, 10, 20),
			*BuildLRP("pg-1", 1, linuxRootFSURL, 10, 20),
			*BuildLRP("pg-2", 0, linuxRootFSURL, 10, 20),
			*BuildLRP("pg-3", 0, linuxRootFSURL, 10, 20),
			*BuildLRP("pg-4", 0, linuxRootFSURL, 10, 20),
		})
	...
	Describe("ScoreForLRP", func() {
		It("factors in memory usage", func() {
			bigInstance := BuildLRP("pg-big", 0, linuxRootFSURL, 20, 10)
			smallInstance := BuildLRP("pg-small", 0, linuxRootFSURL, 10, 10)
		...
		It("factors in disk usage", func() {
			bigInstance := BuildLRP("pg-big", 0, linuxRootFSURL, 10, 20)
			smallInstance := BuildLRP("pg-small", 0, linuxRootFSURL, 10, 10)
		...
		It("factors in container usage", func() {
			instance := BuildLRP("pg-big", 0, linuxRootFSURL, 20, 20)
		...
		It("factors in process-guids that are already present", func() {
			instanceWithTwoMatches := BuildLRP("pg-1", 2, linuxRootFSURL, 10, 10)
			instanceWithOneMatch := BuildLRP("pg-2", 1, linuxRootFSURL, 10, 10)
			instanceWithNoMatches := BuildLRP("pg-new", 0, linuxRootFSURL, 10, 10)
		...
		Context("when the LRP does not fit", func() {
			Context("because of memory constraints", func() {
				It("should error", func() {
					massiveMemoryInstance := BuildLRP("pg-new", 0, linuxRootFSURL, 10000, 10)
			...
			Context("because of disk constraints", func() {
				It("should error", func() {
					massiveDiskInstance := BuildLRP("pg-new", 0, linuxRootFSURL, 10, 10000)
			...
			Context("because of container constraints", func() {
				It("should error", func() {
					instance := BuildLRP("pg-new", 0, linuxRootFSURL, 10, 10)
		...
		Describe("matching the RootFS", func() {
			Context("when the cell provides a complex array of RootFSes", func() {
				...
				It("should support LRPs with various stack requirements", func() {
					score, err := cell.ScoreForLRP(BuildLRP("pg", 0, "fixed-set-1:root-fs-1", 10, 10))
					Expect(score).To(BeNumerically(">", 0))
					Expect(err).NotTo(HaveOccurred())

					score, err = cell.ScoreForLRP(BuildLRP("pg", 0, "fixed-set-1:root-fs-2", 10, 10))
					Expect(score).To(BeNumerically(">", 0))
					Expect(err).NotTo(HaveOccurred())

					score, err = cell.ScoreForLRP(BuildLRP("pg", 0, "fixed-set-2:root-fs-1", 10, 10))
					Expect(score).To(BeNumerically(">", 0))
					Expect(err).NotTo(HaveOccurred())

					score, err = cell.ScoreForLRP(BuildLRP("pg", 0, "fixed-set-2:root-fs-2", 10, 10))
					Expect(score).To(BeNumerically(">", 0))
					Expect(err).NotTo(HaveOccurred())

					score, err = cell.ScoreForLRP(BuildLRP("pg", 0, "arbitrary-1://random-root-fs", 10, 10))
					Expect(score).To(BeNumerically(">", 0))
					Expect(err).NotTo(HaveOccurred())

					score, err = cell.ScoreForLRP(BuildLRP("pg", 0, "arbitrary-2://random-root-fs", 10, 10))
					Expect(score).To(BeNumerically(">", 0))
					Expect(err).NotTo(HaveOccurred())
				})

				It("should error for LRPs with unsupported stack requirements", func() {
					score, err := cell.ScoreForLRP(BuildLRP("pg", 0, "fixed-set-1:root-fs-3", 10, 10))
					Expect(score).To(BeZero())
					Expect(err).To(MatchError(rep.ErrorIncompatibleRootfs))

					score, err = cell.ScoreForLRP(BuildLRP("pg", 0, "fixed-set-3:root-fs-1", 10, 10))
					Expect(score).To(BeZero())
					Expect(err).To(MatchError(rep.ErrorIncompatibleRootfs))

					score, err = cell.ScoreForLRP(BuildLRP("pg", 0, "arbitrary-3://random-root-fs", 10, 10))
					Expect(score).To(BeZero())
					Expect(err).To(MatchError(rep.ErrorIncompatibleRootfs))
				})
			})
			...
			Context("when the cell supports a single RootFS", func() {
				...
				It("should error for LRPs with unsupported stack requirements", func() {
					...
					score, err = cell.ScoreForLRP(BuildLRP("pg", 0, "arbitrary://random-root-fs", 10, 10))
					Expect(score).To(BeZero())
					Expect(err).To(MatchError(rep.ErrorIncompatibleRootfs))
				})
			})
	...
	Describe("ReserveLRP", func() {
		Context("when there is room for the LRP", func() {
			It("should register its resources usage and keep it in mind when handling future requests", func() {
				instance := BuildLRP("pg-test", 0, linuxRootFSURL, 10, 10)
				instanceToAdd := BuildLRP("pg-new", 0, linuxRootFSURL, 10, 10)
			...
			It("should register the LRP and keep it in mind when handling future requests", func() {
				instance := BuildLRP("pg-test", 0, linuxRootFSURL, 10, 10)
				instanceWithMatchingProcessGuid := BuildLRP("pg-new", 1, linuxRootFSURL, 10, 10)
				instanceToAdd := BuildLRP("pg-new", 0, linuxRootFSURL, 10, 10)
		...
		Describe("matching the RootFS", func() {
			Context("when the cell provides a complex array of RootFSes", func() {
				...
				It("should support LRPs with various stack requirements", func() {
					err := cell.ReserveLRP(BuildLRP("pg", 0, "fixed-set-1:root-fs-1", 10, 10))
					Expect(err).NotTo(HaveOccurred())

					err = cell.ReserveLRP(BuildLRP("pg", 0, "fixed-set-1:root-fs-2", 10, 10))
					Expect(err).NotTo(HaveOccurred())

					err = cell.ReserveLRP(BuildLRP("pg", 0, "fixed-set-2:root-fs-1", 10, 10))
					Expect(err).NotTo(HaveOccurred())

					err = cell.ReserveLRP(BuildLRP("pg", 0, "fixed-set-2:root-fs-2", 10, 10))
					Expect(err).NotTo(HaveOccurred())

					err = cell.ReserveLRP(BuildLRP("pg", 0, "arbitrary-1://random-root-fs", 10, 10))
					Expect(err).NotTo(HaveOccurred())

					err = cell.ReserveLRP(BuildLRP("pg", 0, "arbitrary-2://random-root-fs", 10, 10))
					Expect(err).NotTo(HaveOccurred())
				})

				It("should error for LRPs with unsupported stack requirements", func() {
					err := cell.ReserveLRP(BuildLRP("pg", 0, "fixed-set-1:root-fs-3", 10, 10))
					Expect(err).To(MatchError(rep.ErrorIncompatibleRootfs))

					err = cell.ReserveLRP(BuildLRP("pg", 0, "fixed-set-3:root-fs-1", 10, 10))
					Expect(err).To(MatchError(rep.ErrorIncompatibleRootfs))

					err = cell.ReserveLRP(BuildLRP("pg", 0, "arbitrary-3://random-root-fs", 10, 10))
					Expect(err).To(MatchError(rep.ErrorIncompatibleRootfs))
				})
			})
			...
			Context("when the cell supports a single RootFS", func() {
				...
				It("should error for LRPs with unsupported stack requirements", func() {
					...
					err = cell.ReserveLRP(BuildLRP("pg", 0, "arbitrary://random-root-fs", 10, 10))
					Expect(err).To(MatchError(rep.ErrorIncompatibleRootfs))
				})
			})
		})
		...
		Context("when there is no room for the LRP", func() {
			It("should error", func() {
				instance := BuildLRP("pg-test", 0, linuxRootFSURL, 10000, 10)
	...
	Describe("Commit", func() {
		...
		Context("with work to commit", func() {
			...
			BeforeEach(func() {
				lrp = *BuildLRP("pg-new", 0, linuxRootFSURL, 20, 10)
