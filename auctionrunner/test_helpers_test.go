...
func BuildLRPStartRequest(
	...
	return auctioneer.NewLRPStartRequest(
		...
		indices,
		placementTags,
		rep.NewResource(memoryMB, diskMB, rootFS, volumeDriver),
	)
}
...
func BuildLRP(
	...
	lrp := rep.NewLRP(lrpKey, placementTags, rep.NewResource(memoryMB, diskMB, rootFS, []string{}))
	return &lrp
}
...
func BuildLRPAuction(
	...
	return auctiontypes.NewLRPAuction(
		rep.NewLRP(
			lrpKey,
			placementTags,
			rep.NewResource(memoryMB, diskMB, rootFS, volumeDrivers),
		),
		queueTime,
	)
}
...
func BuildLRPAuctions(start auctioneer.LRPStartRequest, queueTime time.Time) []auctiontypes.LRPAuction {
	...
	for _, index := range start.Indices {
		...
		auctions = append(auctions, auctiontypes.NewLRPAuction(
			rep.NewLRP(lrpKey, start.PlacementTags, start.Resource),
			queueTime,
		))
	}
