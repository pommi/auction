...
var _ = Describe("Scheduler", func() {
	...
	Describe("handling start auctions", func() {
			...
			zones["A-zone"] = auctionrunner.Zone{
				auctionrunner.NewCell(
					...
					BuildCellState("A-zone", 100, 100, 100, false, linuxOnlyRootFSProviders, []rep.LRP{
						*BuildLRP("pg-1", 0, "", 10, 10),
						*BuildLRP("pg-2", 0, "", 10, 10),
					}),
				),
			}
		...
		Context("when there is no room", func() {
			BeforeEach(func() {
				startAuction = BuildLRPAuctionWithPlacementError("pg-4", 0, linuxRootFSURL, 1000, 1000, clock.Now(), rep.ErrorInsufficientResources.Error())
				clock.Increment(time.Minute)
				s := auctionrunner.NewScheduler(workPool, zones, clock)
				results = s.Schedule(auctiontypes.AuctionRequest{LRPs: []auctiontypes.LRPAuction{startAuction}})
			})
	...
	Describe("handling task auctions", func() {
		...
		BeforeEach(func() {
			...
			zones["A-zone"] = auctionrunner.Zone{auctionrunner.NewCell("A-cell", clients["A-cell"], BuildCellState("A-zone", 100, 100, 100, false, linuxOnlyRootFSProviders, []rep.LRP{
				*BuildLRP("does-not-matter", 0, "", 10, 10),
				*BuildLRP("does-not-matter", 0, "", 10, 10),
			}))}
